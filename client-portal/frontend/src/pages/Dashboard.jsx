...
import { Menu, X, User, History, CreditCard, Settings, HelpCircle, LogOut, Home, Gift, CheckCircle, Navigation, MapPin, Clock, Star, FileText, Package, Truck, Bike, Car, ChevronRight, ChevronLeft, Info, Utensils, BedDouble, Stethoscope, ShoppingBag } from 'lucide-react'
...
export default function Dashboard({ onNavigate }) {
    ...
    const [recentPlaces, setRecentPlaces] = useState([]) // Histórico de locais

    // Carregar histórico ao iniciar
    useEffect(() => {
        const saved = localStorage.getItem('tot_recent_places')
        if (saved) {
            try {
    ...
    const saveToHistory = (place) => {
        ...
        setRecentPlaces(prev => {
            // Remove duplicatas
            const filtered = prev.filter(p => p.title !== place.title).slice(0, 4) // Mantém top 5
            const newList = [place, ...filtered]
            localStorage.setItem('tot_recent_places', JSON.stringify(newList))
            return newList
        })
    }
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            socketRef.current.on('trip_cancelled', () => {
                console.log('🚨 [TRIP_CANCELLED] Motorista cancelou a corrida');
                resetFlow();
    ...
    const handleSelectSuggestion = async (place) => {
        ...
        if (position) {
            if (activeField === 'origin') {
                ...
                // [PREMIUM] SMART PRECISION LOGIC
                // Only overwrite GPS if the user explicitly selected a DIFFERENT place (distance > 50m)
                // Otherwise, keep the high-precision GPS coords for better pickup.
                if (userPos && userPos.lat && userPos.lng) {
                    const dist = haversine(userPos.lat, userPos.lng, position.lat, position.lng)
                    if (dist > 0.05) { // 50 meters
                        ...
                        console.log('🎯 Keeping precise GPS location despite address selection')
                    }
                } else {
                    // Primeira vez ou GPS não disponível - sempre usar posição selecionada
                    setUserPos({ lat: position.lat, lng: position.lng });
                    addMarker(position, 'origin');
                    if (mapInstance.current) mapInstance.current.setCenter(position);
                }
    ...
    const resetFlow = () => {
        console.log('🔄 [RESET_FLOW] Iniciando reset do fluxo');
        ...
        // DEFENSIVE: Only remove driver marker if it exists and map is ready
        try {
            if (markersRef.current.driver && mapInstance.current) {
                console.log('📋 [RESET_FLOW] Removendo marcador do motorista');
                mapInstance.current.removeObject(markersRef.current.driver);
                markersRef.current.driver = null;
                console.log('✅ [RESET_FLOW] Marcador removido com sucesso');
            } else {
                console.log('⚠️ [RESET_FLOW] Marcador ou mapa não disponível, pulando remoção');
            }
        } catch (error) {
            console.error('❌ [RESET_FLOW] Erro ao remover marcador:', error);
            // Continue anyway - don't let this break the reset
            markersRef.current.driver = null;
        }
        ...
        console.log('✅ [RESET_FLOW] Reset concluído');
    }
    ...
    const handleCategorySearch = async (category) => {
        ...
        let queryTerm = ''
        if (category === 'restaurant') queryTerm = 'restaurant'
        else if (category === 'hotel') queryTerm = 'hotel'
        else if (category === 'hospital') queryTerm = 'hospital'
        else if (category === 'shopping') queryTerm = 'shopping'

        const results = await hereGeocodingService.discoverPlaces(queryTerm, userPos)
        ...
        <div className="dashboard-container">
            ...
            <main className="dashboard-content">
                <div className="welcome-banner">
                    {['ongoing', 'finished'].includes(step) ? (
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            {step === 'sub_service_selection' && (
                                <button className="btn-mini-pink" onClick={() => setStep('service_selection')}>
                                    <ChevronLeft size={14} /> Voltar
                                </button>
                            )}
                            {step === 'destination_input' && (
                                <button className="btn-mini-pink" onClick={() => setStep('sub_service_selection')}>
                                    <ChevronLeft size={14} /> Modalidade
                                </button>
                            )}
                            {step === 'delivery_details' && (
                                <button className="btn-mini-pink" onClick={() => setStep('destination_input')}>
                                    <ChevronLeft size={14} /> Destino
                                </button>
                            )}
                            {step === 'estimating' && (
                                <button className="btn-mini-pink" onClick={() => selectedCategory === 'delivery' ? setStep('delivery_details') : setStep('destination_input')}>
                ...
                <div className={`map-area ${step}`}>
                    {/* MAPA SEMPRE VISÍVEL - Overlay de informações durante viagem */}
                    <HereMap
                        center={userPos}
                        zoom={15}
                    ...
                    {['ongoing', 'finished'].includes(step) && (
                        <div className="trip-details-overlay animate-fade-in">
                        ...
                        <div className="address-card shadow">
                            ...
                            {step === 'destination_input' && (
                                <>
                                    ...
                                    {/* QUICK CATEGORIES - Full Width below dot */}
                                    {activeField === 'destination' && !destAddress && !addressSuggestions.length && (
                                        <div className="quick-categories-row animate-slide-up" style={{ marginTop: '5px', borderRadius: '0 0 20px 20px', borderTop: 'none' }}>
                                            <button className="q-cat-btn" onClick={() => handleCategorySearch('restaurant')}>
                                                <Utensils size={14} /> Restaurantes
                                            </button>
                                            <button className="q-cat-btn" onClick={() => handleCategorySearch('hotel')}>
                                                <BedDouble size={14} /> Hotéis
                                            </button>
                                            <button className="q-cat-btn" onClick={() => handleCategorySearch('hospital')}>
                                                <Stethoscope size={14} /> Hospitais
                                            </button>
                                            <button className="q-cat-btn" onClick={() => handleCategorySearch('shopping')}>
                                    ...
                                    {activeField === 'destination' && (
                                        <div className="address-row" style={{ paddingTop: 0 }}>
                                            <ul className="suggestions-list animate-slide-up" style={{ width: '100%', marginTop: 0 }}>
                                                ...
                                                {addressSuggestions.length > 0 ? (
                                                    ...
                                                    /* LISTA DE HISTÓRICO RECENTE (QUANDO VAZIO) */
                                                    !destAddress && recentPlaces.length > 0 && (
                                                        <>
                                                            <li className="list-header-sm"><History size={12} /> Recentes</li>
                                                            {recentPlaces.map((place, idx) => (
                                                                <li key={`hist-${idx}`} onClick={() => handleSelectSuggestion(place)} className="suggestion-item history">
                                                                    <div className="suggestion-icon history"><Clock size={16} /></div>
                                                                    <div className="suggestion-text">
                                                                        <strong>{place.title}</strong>
                                                                        <small>{place.address}</small>
                                                                    </div>
