...
export const hereRoutingService = {
     ...
     */
    async getRoute(start, end, transportMode = 'car') {
        // Validation Guard
        if (!start || !end) {
            console.warn('⚠️ Routing skipped: Missing start or end point.', { start, end });
            return null;
        }
        if (
            ...
            console.warn('⚠️ Routing skipped: Invalid coordinates.', { start, end });
            return null;
        }
        ...
        // ATUALIZAÇÃO PROFISSIONAL: Adicionado actions, instructions e turnByTurnActions para navegação
        ...
        try {
            ...
            return {
                geometry,
                actions: section.actions, // Manobras detalhadas para navegação
